<template>
    <router-link class="nft-preview" v-bind:to="route">
        <div v-if="invalid" class="nft-preview__image">
            <div class="nft-preview-error">
                <div class="nft-preview-error__info">
                    <svg v-pre class="nft-preview-error__icon" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24">
                ...
                <div class="nft-preview-error__button"
                    v-on:click.prevent.stop="$emit('reload', index)"
                    v-text="$t('nft.item.preview.error_button')"/>
            </div>
        </div>

        <img v-else-if="images.w320" class="nft-preview__image"
